...
///gioi han vung rect chi cho phep nam trong vung anh
template <typename t>
...
///check va lay vung cua so nam trong anh
inline cv::Mat subwindow(const cv::Mat &in, const cv::Rect &window, int borderType = cv::BORDER_CONSTANT) {
    ...
    if (cutWindow.height <= 0 || cutWindow.width <= 0) {
        return cv::Mat(window.height, window.width, in.type(), nullptr);
    }
    cv::Rect border = RectTools::getBorder(window, cutWindow);
    cv::Mat res(window.width, window.height, CV_32FC3);
    res = in(cutWindow);
